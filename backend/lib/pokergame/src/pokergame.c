...
//   int i = 0;
//   int j = 0;
//   int setloop = 1;   /*a set is a completely new redraw from a full deck. keep playing new sets until only one player
//   has all the money*/ int roundloop = 1; /*each set is made up of 2 rounds. players left after the 2nd round reveal
//   their cards*/ int numbankruptplayers = 0; int numfolded = 0;
...
//   int maxcumulbet = 0; /*the cumulative max amount in the pool by any one player. this is the minimum cumulative bet
//   in round 2 for all other players.*/

//   // variables to hold logic to act on. Generated by Monte Carlo advisor
//   int *maxdiscards = (int *)malloc(sizeof(int) * HAND_SIZE); /*index of the cards in players[i]->hand that Monte
//   Carlo advisor thinks player should discard*/ int *maxthrown = (int *)malloc(sizeof(int));               /*number of
//   cards Monte Carlo advisor thinks the player should discard*/ int *maxvalue = (int *)malloc(sizeof(int)); /*highest
//   expected value out of all 32 discard options available. Used by AI to determine a bet amount. */ int adiscard = 0;

//   int maxplayer = 0;      /*holds the index corresponding to the player with the highest value hand in Player **
//   players */ int maxplayervalue = 0; /*holds the highest value hand found while iterating over all the players*/

//   char str[] = "0";

//   srand(time(NULL));
//   printf("\n\n\n\n========[Welcome to Five Card Monte Carlo Poker]========\n\n");
//   printf("Rules:\n(A) 2 rounds of betting per set. Bets go into the 'pool'.\n(B) Every player must call the highest
//   bet after round 1 in order to remain in the game.\n(C) In each round, every player may discard and draw up to 5
//   cards from the deck.\n(D) A player may also choose to fold at any time, but forfeits any bets they already
//   made.\n(E) Player with the highest set of cards at the end of 2 rounds wins all the money in the pool.\n(F)The game
//   continues in sets of 2 rounds until all players except one is bankrupt.\n");
...
//               for (j = 0; j < HAND_SIZE; j++)
//               {
//                 printf("Enter the number[1-5] of ONE card you wish to discard, or 0 if done discarding, or 6 to
//                 fold:"); scanf("%d", &adiscard);
...
//     // find the player with the highest card and award them the pool, when there is more than one player remaining at
//     the end of two rounds if (numfolded != NUMPLAYERS - 1)
//     {
...
//     for (i = 0; i < NUMPLAYERS; i++)
//     {
//       if (players[i]->money != 0) // only reset players fold status if they have money. keep them folded if they are
//       bankrupt
//         players[i]->fold = 0;
//       players[i]->bet = 0;
...
//   // printf("\ncomputer: maxbet:%f, bet:%d, expval:%d, maxcumulbet:%d, playerbet:%d,
...
//   { // if the incremental bet isn't higher than what is needed to call, but the player is the highest bet already,
//   stay
//     return 0;
//   }
//   else
