...
void game_state_reset_for_new_hand(GameState* state)
{
    ...
    for (int i = 0; i < MAX_PLAYERS; i++)
    {
        if (state->players[i].state != PLAYER_STATE_EMPTY && state->players[i].state != PLAYER_STATE_SITTING_OUT)
        {
...
{
    if (!state || seat < 0 || seat >= MAX_PLAYERS)
        return -1;
    if (state->players[seat].state == PLAYER_STATE_EMPTY)
        return -2;
...
GamePlayer* game_get_player_by_id(GameState* state, int player_id)
{
    ...
    for (int i = 0; i < MAX_PLAYERS; i++)
    {
        if (state->players[i].state != PLAYER_STATE_EMPTY && state->players[i].player_id == player_id)
        {
...
int game_count_active_players(GameState* state)
{
    ...
    for (int i = 0; i < MAX_PLAYERS; i++)
    {
        if (state->players[i].state == PLAYER_STATE_ACTIVE || state->players[i].state == PLAYER_STATE_ALL_IN)
        {
            count++;
        }
    }
...
{
    if (!state || state->hand_in_progress)
        return -1;
    if (state->num_players < 2)
        return -2;
...
int game_deal_flop(GameState* state)
{
    if (!state || !state->deck)
        return -1;
    if (state->num_community_cards != 0)
        return -2;
...
int game_deal_turn(GameState* state)
{
    if (!state || !state->deck)
        return -1;
    if (state->num_community_cards != 3)
        return -2;
...
int game_deal_river(GameState* state)
{
    if (!state || !state->deck)
        return -1;
    if (state->num_community_cards != 4)
        return -2;
...
bool game_is_betting_round_complete(GameState* state)
{
    ...
    if (active_count <= 1)
        return true;
    if (all_matched && state->last_aggressor_seat != -1)
        return true;
...
void game_get_available_actions(GameState* state, int player_id, AvailableAction* actions, int* num_actions)
{
    *num_actions = 0;
    ...
    {
        // Can bet
        actions[(*num_actions)++] = (AvailableAction) {ACTION_BET, state->big_blind, player->money, state->big_blind};
    }
    else if (state->current_bet > 0)
    {
        ...
        if (player->money + player->bet > min_raise_total)
        {
            // Can raise
            actions[(*num_actions)++] =
                (AvailableAction) {ACTION_RAISE, min_raise_total, player->money + player->bet, state->big_blind};
        }
    }
...
int game_determine_winner(GameState* state)
{
    ...
    for (int i = 0; i < MAX_PLAYERS; i++)
    {
        ...
        if (p->state == PLAYER_STATE_ACTIVE || p->state == PLAYER_STATE_ALL_IN)
        {
            ...
            if (p->hole_cards[0])
                add_card(&temp_hand, p->hole_cards[0]);
            if (p->hole_cards[1])
                add_card(&temp_hand, p->hole_cards[1]);
...
void game_set_dealer_button(GameState* state)
{
    ...
    for (int i = 0; i < MAX_PLAYERS; i++)
    {
        ...
        if (state->players[seat].state != PLAYER_STATE_EMPTY && state->players[seat].state != PLAYER_STATE_SITTING_OUT)
        {
